...
download() {
  ...
  curl | * | "")
    echo -e "Downloader: \e[32mcurl\e[0m"
    echo -e "\033[1;36m$2 => \033[1;33m$1\033[0m"
    if curl ${curl_arg[@]} --progress-bar -f -L -C - -o $temp_file "$1"; then # <== 必须有-L参数否则无法重定向链接解析不出来
      mv "$temp_file" "$2"
    else
    ...
    ;;
  wget)
    echo -e "Downloader: \e[32mwget\e[0m"
    echo -e "\033[1;36m$2 => \033[1;33m$1\033[0m"
    if wget ${wget_arg} -O $temp_file -c "$1"; then
      mv "$temp_file" "$2"
    else
    ...
    ;;
  aria2c)
    echo -e "Downloader: \e[32maria2c\e[0m"
    echo -e "\033[1;36m$2 => \033[1;33m$1\033[0m"
    if [[ -z $useThread ]]; then
      local useThread=$(nproc)
    fi
    if aria2c ${aria2c_arg} -x $useThread -s $useThread -c -o $(basename $temp_file) -d $TMPDIR/ "$1"; then
      mv "$temp_file" "$2"
    else
