...
if [[ -z $returnFileListName ]] || [[ $BACK_NUM == E ]]; then
    . ~/NumBox/Numbox
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            if ! load "rm -rf ~/NumBox/data/container/${returnFileListName}" "正在删除容器"; then
                echo 部分或所有容器删除失败
                return 1
            fi
        else
...
elif [[ -d ~/NumBox/data/container/${returnFileListName} ]]; then
    dialog --colors --erase-on-exit --no-kill --title "是否删除容器？" --yes-label "否" --no-label "删除" --yesno "容器名：\Z3$returnFileListName\Zn 大小/路径：$(du -sh ~/NumBox/data/container/${returnFileListName})" $box_sz2
    if [[ $? == 1 ]]; then
        if ! load "rm -rf ~/NumBox/data/container/${returnFileListName}" "正在删除容器"; then
            echo 容器删除失败
            return 1
