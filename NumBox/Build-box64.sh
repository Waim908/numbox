...
main() {
  ...
  build_box64_menu() {
    build_cmake() {
      ...
      2)
        ...
        ;;
      3)
        export CFLAGS="-O3 -marh=native -flto=auto"
    ...
    build_box64_sub_menu() {
      ...
      c) main ;;
      d)
        Dyesno "不需要安装吗？" "构建完成后你可以后续选择安装对应源码包构建的版本"
        if [[ $? == 0 ]]; then
        ...
        ;;
      1)
        local cmake_arg+=(-DBAD_SIGNAL=on)
        build_cmake
        ;;
      2)
        local cmake_arg+=(-DBAD_SIGNAL=on -DSD8G2=on)
        build_cmake
        ;;
      3)
        local cmake_arg+=(-DBAD_SIGNAL=on -DSD888=on)
        build_cmake
        ;;
      4)
        local cmake_arg+=(-DBAD_SIGNAL=on -DSD865=on)
        build_cmake
        ;;
      5)
        local cmake_arg+=(-DBAD_SIGNAL=on -DSD845=on)
        build_cmake
        ;;
      6)
        local cmake_arg_arry=(-DBAD_SIGNAL=on -DNOLOADADDR=on -DSD8G2=on -DSD888=on -DSD865=on -DSD845=on)
        ...
        *)
          ...
          Dinputbox "自定义CMake参数" "当前: ${add_cmake_arg[@]}\n输入no取消构建\n格式:-DXXX=on/off"
  ...
  ghRepo=box64
  ghUser=ptitSeb
  Dmenu_select=(1 从本地源码构建 2 从本地源码安装 3 构建稳定版-偶数-全部 4 构建开发版-奇数-最新 5 从指定提交构建-git)
  Dmenu "Box64版本构建与安装(glibc)" "不会获取到历史奇数版本开发版"
  case $DMENU in
  "") last_jump ;;
  1)
    file_list "/sdcard/NumBox/resources/box64/src/" "从本地源码构建(.tar.gz/.zip包)" "/sdcard/NumBox/resources/box64/src/"
    case $returnFileListNum in
    "") main ;;
    *) if [[ $returnFileListName == *.tar.gz ]]; then
      if ! load "un_tgz /sdcard/NumBox/resources/box64/src/\"$returnFileListName\" $TMPDIR/box64_src_output" "解压文件"; then
        warn "文件解压失败"
      else
        build_box64_menu
      fi
    elif [[ $returnFileListName == *.zip ]]; then
      if ! load "un_tgz /sdcard/NumBox/resources/box64/src/\"$returnFileListName\" $TMPDIR/box64_src_output" "解压文件"; then
        warn "文件解压失败"
      else
        build_box64_menu
      ...
      warn "不支持的文件格式/sdcard/NumBox/resources/box64/src/$returnFileListName"
      return 1
    fi ;;
    esac
    ;;
  2)
    file_list "/sdcard/NumBox/resources/box64/src_output/" "从本地源码安装(.tar.xz包)" "/sdcard/NumBox/resources/box64/src_output/"
    case $returnFileListNum in
    "") main ;;
    *) if ! load "un_txz /sdcard/NumBox/resources/box64/src_output/\"$returnFileListName\" $TMPDIR/box64_src_output"; then
      warn "文件解压失败"
    else
      cd $TMPDIR/box64_src_output/*/
      if ! cd build; then
  ...
  3)
    ...
    ;;
  4)
    src_url="https://github.com/ptitSeb/box64/archive/refs/heads/main.zip"
    if [[ -z $ghSpeed ]] || [[ $ghSpeed == github.com ]]; then
      info "没有设置github加速站"
    else
      info "加速站设置为$ghSpeed"
    ...
    ;;
  5)
    Dinputbox "输入完整的github commit提交hash"
    if [[ -z $DINPUTBOX ]]; then
      main
    else
      cd $TMPDIR/
      ...
      if ! git clone -b "DINPUTBOX" $src_url box64_src_output; then
        warn "克隆提交：$DINPUTBOX 时出现错误！"
        return 1
      else
        build_box64_menu
      fi
